...
export default class HyperList {
  ...
  _getRow (i) {
    ...
    const scrollAxis = config.horizontal ? 'left' : 'top'
    const crossAxis = config.horizontal ? 'height' : 'width'
    item.style.cssText += `
      position: absolute;
      ${scrollAxis}: ${position}px;
      ${crossAxis}: 100%;
    `
  ...
  _computePositions (from = 0) {
    ...
    if (from == 0) {
      itemPositions[0] = reverse ? this._scrollHeight - itemHeights[0] : 0
      from += 1
    }
    if (reverse) {
      for (let i = from; i < itemCount; i++) {
        itemPositions[i] = itemPositions[i - 1] - itemHeights[i]
      }
    } else {
      for (let i = from; i < itemCount; i++) {
